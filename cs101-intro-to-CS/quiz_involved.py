...
def involved(courses, person):
    # Should have been broken into smaller procedures to clean up this cascading bullshit
    result = {}
    for semester in courses:
        coursesOffered = courses[semester]
        for course in coursesOffered:
            courseOffered = coursesOffered[course]
            for attribute in courseOffered:
                if courseOffered[attribute] == person:
                    if semester in result:
                        result[semester].append(course)
                    else:
                        result[semester] = [course]
