...
def crawl_web(startPage,max_depth):
    tocrawl = [startPage]
    crawled = []
    '''
    Add a depth variable to keep track of the depth (obviously)
    We define a nextLevelToBeCrawled array to store the contents of the next next level, 
    This means that we will first crawl all the links in level n, then move to level (n+1)
    '''
    depth = 0
    nextLevelToBeCrawled = []

    while tocrawl:
        page = tocrawl.pop()
        if page not in crawled and (depth <= max_depth):
            union(nextLevelToBeCrawled, get_all_links(get_page(page)))
            crawled.append(page)
        if len(tocrawl) == 0:
            tocrawl = nextLevelToBeCrawled
            nextLevelToBeCrawled = []
            depth += 1
    return crawled
...
#>>> ['http://top.contributors/forbiddenvoid.html', 'http://top.contributors/jesyspa.html', 'http://top.contributors/velak.html'] 
